defmodule PortfolioWeb.AdminAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_portfolio_web_admin_remember_me"
  ...
  def fetch_current_admin(conn, _opts) do
    ...
    admin = admin_token && Backoffice.get_admin_by_session_token(admin_token)
  ...
  the current_admin:
      ...
      defmodule PortfolioWeb.PageLive do
        ...
        on_mount {PortfolioWeb.AdminAuth, :mount_current_admin}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{PortfolioWeb.AdminAuth, :ensure_authenticated}] do
  ...
  defp mount_current_admin(socket, session) do
    Phoenix.Component.assign_new(socket, :current_admin, fn ->
      if admin_token = session["admin_token"] do
        Backoffice.get_admin_by_session_token(admin_token)
      end
    end)
  end
